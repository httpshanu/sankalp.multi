...
import { CheckCircle2, Save, Send, ChevronRight, ChevronLeft, Plus, Trash2 } from 'lucide-react';

function generateId() {
  return 'SN-' + Date.now() + '-' + Math.random().toString(36).substr(2, 4);
}

export default function NewPatientNurse() {
  ...
  const [formData, setFormData] = useState({
    ...
    babies: [{ baby_number: 1, gender: 'male', uhid: '', gestational_age: '', birth_weight: '', outcome_of_delivery: 'live_birth' }],
  });
  ...
  useEffect(() => {
    if (id) {
      ...
      if (patient) {
        const mappedData = {
          identification: {
            district: patient.district || 'Haridwar',
            block_name: patient.block_name || '',
            facility_type: patient.facility_type || 'public',
            ...
            nurse_id: patient.nurse_id || user?.id,
            nurse_name: patient.nurse_name || user?.name,
          },
          ...
          })) || [{ baby_number: 1, gender: 'male', uhid: '', gestational_age: '', birth_weight: '', outcome_of_delivery: 'live_birth' }],
        };
  ...
  const addBaby = () => {
    setFormData(prev => {
      ...
      return {
        ...prev,
        identification: { ...prev.identification, baby_count: prev.babies.length + 1 },
        babies: [...prev.babies, {
          baby_number: newBabyNumber, gender: 'male', uhid: '',
          gestational_age: '', birth_weight: '', outcome_of_delivery: 'live_birth',
        }],
      };
    });
  };
  ...
  const handleSaveDraft = () => {
    const patientData = buildPatientData('with_nurse');
    upsertPatient(patientData);
    addAuditLog({
      user_id: user?.id, user_name: user?.name, role: user?.role,
      action: 'created', record_id: patientData.id,
    });
    setSaved(true);
  ...
  const handleHandover = () => {
    const patientData = buildPatientData('submitted_to_supervisor');
    upsertPatient(patientData);
    addAuditLog({
      user_id: user?.id, user_name: user?.name, role: user?.role,
      action: 'handed_over', record_id: patientData.id,
    });
    setSubmitted(true);
    setTimeout(() => navigate('/nurse/patients'), 2000);
  };
    ...
    <AppLayout title={id ? t('editPatient') : t('patientRegistration')}>
      <div className="max-w-3xl mx-auto">
                    ...
                    <button key={n} type="button" onClick={() => {
                      ...
                      setFormData(prev => {
                        ...
                        if (currentBabies.length < count) {
                          ...
                          return {
                            ...prev,
                            identification: { ...prev.identification, baby_count: count },
                  ...
                  <div key={idx} className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="col-span-2">
                        <label className="block text-sm font-medium text-slate-700 mb-1">{t('outcomeOfDelivery')}</label>
                        <div className="flex gap-4 flex-wrap">
                          {[
                            { value: 'live_birth', label: t('liveBirth') },
                            { value: 'still_birth', label: t('stillBirth') },
                            { value: 'abortion', label: t('abortion') },
                          ].map(o => (
                            <label key={o.value} className="flex items-center gap-2 cursor-pointer">
                              <input type="radio" name={`baby_outcome_${idx}`} value={o.value} checked={baby.outcome_of_delivery === o.value} onChange={() => updateBaby(idx, 'outcome_of_delivery', o.value)} />
        ...
        <div className="flex items-center justify-between">
          <button onClick={() => step > 1 ? setStep(s => s - 1) : navigate('/nurse')}
