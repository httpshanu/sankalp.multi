...
import {
  getAllPatients, getBabiesByPatient, getFollowupsByPatient,
  addRemark
} from '../../data/dataStore';
  ...
  return (
    <AppLayout title={t('allPatients')}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800">{t('allPatients')}</h1>
          <p className="text-slate-500 text-sm">{filtered.length} {t('totalPatients')}</p>
        </div>
        <button
          onClick={() => {
            // Admin can't create patients directly, but we can show a message or redirect to supervisor/counsellor creation
            // For now, we'll just show an alert or do nothing.
            alert('Admins cannot create new patients. Please ask a supervisor or counsellor to register a patient.');
          }}
          className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-semibold text-white transition-all hover:opacity-90"
          style={{ background: 'linear-gradient(135deg,#6B46C1,#9F7AEA)' }}
        >
          <Plus size={18} />
          {t('addPatient')}
        </button>
